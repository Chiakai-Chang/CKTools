...
* **辦案累了？來點領先者的「核心眼界」吧！**
* **「科偵之夜 (DeepReadAI)」** 秉持費曼學習法（Feynman Technique）以教為學的靈魂，將全球尖端技術脈動，轉化為 **10 分鐘就能同步的中文精華導讀**。這不是額外的負擔，而是一場讓您在靜默中完成的「降維進化」。
* **💡 打造個人的專業護城河**：
...
* ## ☆ 自動化幫忙「報超勤」密技 ☆
  * 步驟:
    ...
    * 5、程式會自動偵測您的姓名、帳號與單位，並自動偵測當月有幾天可以報超勤，您確認後按下「確定」按鈕，就會自動幫您將可報超勤的日期，每天8小時以後的時數，逐一幫您提報請領加班費的動作，全部完成後會跳出視窗通知，此時重新整理頁面或再按一次「查詢」按鈕，即可發現報超勤工作已完成。
  * 指令:
    ```javascript
    (async function() {
        console.clear();
        console.log("🤖 啟動全自動偵測模式 (相對路徑版本)...");
        ...
        // --- 4. 逐日處理迴圈 (使用相對路徑) ---
        let successCount = 0;
        const servletUrl = "OC07Servlet"; // 因為目前在 /NM106-508Web/ 下，直接呼叫同層的 Servlet
        ...
        for (const dateStr of targetDates) {
            ...
            try {
                // A. 查詢該日上班時段
                const queryResp = await fetch(servletUrl, {
                ...
                // C. 送出儲存
                const saveResp = await fetch(servletUrl, {
                    "headers": { "content-type": "application/x-www-form-urlencoded; charset=UTF-8" },
...
function autoExpandContent() {
    ...
    // 檢查是否需要切換 "所有留言"，否則直接展開內容
    if (!switchToAllComments()) {
        console.log("直接執行展開內容...");
        executeActions();
    }
}
...
* ## ☆ 臺中市公文系統自動校正常見錯誤 ☆
  ...
  * 使用步驟:
      ...
      * 2、請在該頁面按鍵盤上的「**F12**」按鈕來開啟「**開發人員工具**」(即 **DevTools**)
      * 3、請在「**開發人員工具**」(即 **DevTools**) 跳到「**Console**」頁籤，點上方「 **∅** 」符號清空畫面以利觀看程式進度
      * 4、複製以下指令後，到在「**Console**」內貼上後按「**Enter**」執行即可
           * (※ 若無法貼上，請參考 [**allow pasting 的教學**](https://chiakai-chang.github.io/CKTools/#%E5%BF%85%E9%A0%88%E5%85%88%E5%81%9A-%E8%AB%8B%E5%85%88%E6%89%8B%E5%8B%95%E8%BC%B8%E5%85%A5%E4%BB%A5%E4%B8%8B%E6%8C%87%E4%BB%A4%E6%89%8D%E6%9C%83%E8%A2%AB%E5%85%81%E8%A8%B1%E5%9C%A8-devtools-%E7%9A%84-console-%E5%85%A7%E8%B2%BC%E4%B8%8A%E7%A8%8B%E5%BC%8F%E7%A2%BC))
      * 5、完成後，記得回到「**文書編輯**」頁面檢查；若檢查OK，請記得按左上角的「**💾儲存**」按鈕來儲存修改後結果唷！
